...
[![CI](https://github.com/najasinis/nestjs-tenant-shield/actions/workflows/ci.yml/badge.svg)](https://github.com/najasinis/nestjs-tenant-shield/actions/workflows/ci.yml)
...
**v0.2.0 배포됨.** TypeORM + Prisma + BullMQ 지원.

| 버전 | 내용 | 상태 |
|------|------|------|
| v0.1 | TypeORM discriminator 패턴, 미들웨어, 캐시 격리 | ✅ |
| v0.2 | Prisma 어댑터, BullMQ `@TenantContext` + `withTenantPayload` | ✅ |
| v0.3 | Postgres RLS 통합 | ⏳ |
...
상세는 [SECURITY.md](./SECURITY.md) 참조.
...
- `@RequireTenant()` 데코레이터
- TypeORM Subscriber (auto WHERE 주입)
- `@Cacheable({ tenantScoped })` 캐시 격리
- `runWithTenant()` / `runWithoutTenant()` 헬퍼
- examples/academy-saas 데모

### v0.2 ✅
- Prisma 어댑터 (`createTenantAwarePrisma`)
- BullMQ 통합 (`@TenantContext`, `withTenantPayload`)
- GitHub Actions CI/CD
- SECURITY.md, CHANGELOG.md, CONTRIBUTING.md

### v0.3 ⏳
- Postgres RLS 통합 (행 수준 보안)
...
이슈와 PR 환영. [CONTRIBUTING.md](./CONTRIBUTING.md) 참조.
...
- [README](README.md): 사용법/예제/빠른 시작 (이 문서)
- [SECURITY.md](SECURITY.md): 자동 보호 범위·한계·취약점 제보
- [CHANGELOG.md](CHANGELOG.md): 버전별 변경 내역
- [CONTRIBUTING.md](CONTRIBUTING.md): 기여 가이드
- [docs/FAQ.md](docs/FAQ.md): 자주 묻는 질문
- [docs/tenant-shield-api-spec.md](docs/tenant-shield-api-spec.md): API 상세 명세
- [docs/tenant-shield-PRD.md](docs/tenant-shield-PRD.md): 설계/로드맵/배경
