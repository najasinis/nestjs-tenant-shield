...
| **다음 체크포인트** | 데모 GIF 제작 → GitHub Discussions 오픈 → 커뮤니티 포스팅 시작 |
| **Gate B 상태** | ❌ 미달성 — 외부 피드백 0건 / 파일럿 0건 |
| **다음 회고 시점** | 평가 20건 누적 시 `/eval-review` (Opus) |
| **블로커** | 없음 — Go-to-Market 문서·하드닝 게이트·CI 매트릭스 완료 |
| **마지막 갱신** | 2026-06-25 |
| **테스트 현황** | 20 suites / 107 passed / 5 skipped (TSC clean) — 하드닝 6 suites 포함, NestJS 10/11 양쪽 통과 확인 |
...
### 9-0 ✅ Go-to-Market 준비 (2026-06-25 완료)

마케팅 전 필수 준비를 이번 세션에서 완료했다.

**완료된 항목:**

| 항목 | 파일/스크립트 | 비고 |
|---|---|---|
| Go-to-Market 전략 문서 | [docs/go-to-market.md](./go-to-market.md) | Part A(마케팅), Part B(게이트), Part C(예상 문제점), 부록(템플릿·체크리스트) |
| 안정성 게이트 하드닝 테스트 (B-1~B-6b) | `test/hardening/` (6 spec, 26 tests) | B-1 fuzz·B-2 동시성·B-3 property·B-4 쓰기차단·B-5 정직성·B-6b soak — 전부 통과 |
| CI 버전 매트릭스 확장 (B-6a) | `.github/workflows/ci.yml` | Node 18/20/22 × NestJS 10/11 = 6 조합, hardening job 신설 |
| `package.json` repository.url 수정 | — | `jinyeongjung` → `najasinis` (링크 깨짐 방지) |
| C-1.11 보안 버그 수정 | `src/typeorm/tenant.subscriber.ts` | QueryBuilder DELETE/UPDATE 명시 다른 tenantId → CrossTenantAccessError Fail-Loud |
| devDep 추가 | `@faker-js/faker@^8`, `fast-check@^3` | 하드닝 테스트 전용 |

**이번 세션 중 발견·수정한 보안 이슈 (C-1.11)**:
- **증상**: `createQueryBuilder().delete().where('tenantId = :t', { t: otherTenant })` 실행 시 다른 tenant 데이터 삭제됨
- **원인**: `hasTenantWhere` 체크가 "값 일치" 미검증 → `applyTenantWhereForMutation`으로 분리, WHERE 파라미터 값 검사 추가
- **결과**: DELETE/UPDATE에서 현재 컨텍스트와 다른 tenantId 명시 시 즉시 `CrossTenantAccessError`

**남은 사전 자산 (브라우저/녹화 도구 필요):**
- [ ] **데모 GIF/저장소**: "수동 코드 → 누출, 라이브러리 → CrossTenantAccessError throw" 1분 시연 — 가장 강력한 마케팅 자산
...
> **진입 조건**: 9-0 사전 자산 전부 체크 후 진입. 특히 데모 GIF 없이는 보류.

체크리스트:
- [ ] **dev.to (영문 포스트)**
  - 제목 예: "Automatic multi-tenant data isolation in NestJS with one decorator"
  - 초안: [docs/post-devto.md](./post-devto.md)
...
- [ ] **긱뉴스 (한국)**
  ...
  - 초안: [docs/post-geeknews.md](./post-geeknews.md)
...
| 항목 | 상태 | 마지막 확인 |
|---|---|---|
| 타입체크 (`tsc --noEmit`) | ✅ 깨끗 | 2026-06-25 |
| Jest (전체) | ✅ 20 suites / 107 passed / 5 skipped (PG: Docker skip) | 2026-06-25 |
| Jest (하드닝) | ✅ 6 suites / 26 passed — NestJS 10/11 양쪽 확인 | 2026-06-25 |
| CI 매트릭스 | ✅ Node 18/20/22 × NestJS 10/11 = 6 조합 (hardening job 포함) | 2026-06-25 |
| origin/main 동기화 | ⏳ 미push — 이번 세션 로컬 커밋 다수 대기 | 2026-06-25 |
| npm publish | ✅ v0.2.1 | 2026-05-24 |
| 벤치마크 실측 | ✅ ~0.00069 ms/call (Node.js v24.13.1, 100,000 iter) | 2026-05-31 |
| npm fresh install 검증 | ✅ 패키지 설치 + exports 확인 완료 | 2026-05-31 |
| 영문 README | ✅ Quick Start + Honest Limits + API Reference + badges | 2026-05-31 |
| 예제 실코드 (prisma/bullmq/redis) | ✅ 3개 예제 모두 src + package.json + README 완성 | 2026-05-31 |
| Go-to-Market 문서 | ✅ docs/go-to-market.md — Part A·B·C + 부록 | 2026-06-25 |
| 안정성 게이트 (B-1~B-6b) | ✅ 전부 통과 | 2026-06-25 |
| C-1.11 보안 수정 | ✅ DELETE/UPDATE 명시 cross-tenant → CrossTenantAccessError | 2026-06-25 |
| 데모 GIF | ❌ 미제작 | — |
| GitHub Discussions | ❌ 미오픈 | — |
| Gate B (외부 피드백) | ❌ 0 / 3건 | — |
...
| 2026-06-25 | docs/go-to-market.md 추가 — 마케팅 전략(Part A), 안정성 게이트 스펙(Part B), 예상 문제점(Part C), 부록(템플릿·체크리스트). Part B 하네스 코드 구현은 별도 승인 후 진행. |
| 2026-06-25 | Part B 하네스 구현 완료 — test/hardening/ 6개 spec 추가, devDep(@faker-js/faker@^8, fast-check@^3) 설치, test:hardening 스크립트 추가, package.json repository.url 수정(jinyeongjung→najasinis). B-1~B-5·B-6b 전부 통과. 신규 발견: QueryBuilder DELETE 명시 tenantId 미보호(C-1.11 추가). |
| 2026-06-25 | C-1.11 수정 — applyTenantWhereForMutation 추가. DELETE/UPDATE QueryBuilder에서 다른 tenant의 tenantId를 WHERE에 명시하면 CrossTenantAccessError를 던짐(Fail-Loud). honesty.spec.ts 테스트 2개 추가(DELETE/UPDATE C-1.11 케이스). 하드닝 6 suites 26/26 통과. |
| 2026-06-25 | B-6a CI 매트릭스 확장 — .github/workflows/ci.yml에 nestjs-version: [10, 11] 추가 (Node 3 × Nest 2 = 6 조합). hardening job 신설(B-1~B-6b 6 suites). build job이 test + hardening 양쪽 의존. 로컬 Nest 10/11 양쪽 107 passed 확인. |
